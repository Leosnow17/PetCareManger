...
from . import db
from .models import User, Post, Setup, Item, Game
...
def games():
    q = request.args.get('q')
    if q:
        games_q = Game.query.filter(Game.name.contains(f'{q}'))
    else:
        games_q = Game.query.all()
    return render_template('games.html', data=games_q)
